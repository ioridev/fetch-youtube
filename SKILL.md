...
Run `./openclaw-youtube-transcript --url <youtube-url>` for a single video.
Run `./openclaw-youtube-transcript --channel <channel-id-or-handle> --hours <n>` for recent channel videos.
Run `./openclaw-youtube-transcript --config config/channels.example.json --daily` for batch mode.

The main script is `scripts/openclaw_youtube_transcript.py`.
